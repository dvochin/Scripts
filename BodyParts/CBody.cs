...
public class CBody : IHotSpotMgr { 		// Manages a 'body':  Does not actually represent a mesh (most of the skinned body represtend by _oBodySkinnedMesh;
	...
	//public bool					_bIsCumming_OBS;		// Character is currently cumming.  Set globally
	...
	//public List<CSoftBody>	    _aSoftBodies	= new List<CSoftBody>();		//###DESIG: Use?  // List of all our _oSoftBodies_XXX below... used to simplify iterations.
    ...
    public CBody(CBodyBase oBodyBase) {
		...
		CBone.BoneUpdate_UpdateFromBlender(_oBodyBase._sMeshSource, _oBodyBase._oBoneRootT, ref _mapBonesFlattened);
		...
		if (_oBodyBase._eBodySex != EBodySex.Woman) {
	        ...
	        _oPenisColliderPhysX = (CBSkinBaked_PenisMeshCollider)CBMesh.Create(null, _oBodyBase, ".oBody.oMeshPenisColliderPhysX_HACK", typeof(CBSkinBaked_PenisMeshCollider));
			_oPenisColliderPhysX.name = _oBodyBase._sBodyPrefix + "-PenisColliderPhysX";
		}
		if (_oBodyBase._eBodySex == EBodySex.Woman) {       // Only the woman has a (working) vagina		//###TODO24:!! Should be if non-man but working vagina on shemale too tough right now!
			_oVaginaRaycaster = new CVaginaRaycaster(this);         
		}
		if (_oBodyBase._eBodySex != EBodySex.Man) {         // Non-man (woman and shemale) all have breasts
	...
	public void OnSimulatePre() {       //###DESIGN:!!! Redo fucking update mechanism!! ###CLEANUP
		//     if (_oBSkinRim != null)
