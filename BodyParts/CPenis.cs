...
- Collision of dick against body col needs work: Both dick and simulated pants cannot go through each other.
	- We need to tell bodycol to always include inner thighs to repell penis from legs
	- BodyCol generation needs to be smartend up to just one separated cloth?? (or does it do that already)
	- Need to update bodycol at each frame...  different context then cloth-fit mode but we need to integrate the two!
...
- Remember the hack with bodycol deep copy!
...
public class CPenis : CBSoft, IObject, IHotSpotMgr {
	//---------------------------------------------------------------------------	MEMBERS
