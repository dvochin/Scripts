...
public class CBSoft_OldPhysX2 : CBMesh, IObject {					//####DEV ####DESIGN: Based on CBMesh or CBSkin??
	...
	public CBSoft_OldPhysX2() {                           // Setup the default arguments... usually overriden by our derived class   //###BUG??? Why are these settings not overriding those in instanced node???
		_nRangeTetraPinHunt = 0.025f;
	}

	public static CBSoft_OldPhysX2 Create(CBody oBody, Type oTypeBMesh) { 
		...
		CBSoft_OldPhysX2 oBSoft = (CBSoft_OldPhysX2)CBMesh.Create(null, oBody, "aSoftBodies['" + sNameSoftBody + "'].oMeshSoftBody", oTypeBMesh);		// Create the softbody mesh from the just-created Blender mesh.
		return oBSoft;
	}
	...
	public override void OnDestroy() {
		Debug.Log("Destroy CBSoft_OldPhysX2 " + gameObject.name);
		...
		switch (eGameModeNew) { 
			case EGameModes.Play:
				...
				if (nVertTetras > CBody.C_Unity2Blender_MaxVerts)			// Unity to Blender mesh created at a fixed size with the max number of verts we're expecting.  Check if we're within our set limit
					throw new Exception("ERROR in CBSoft_OldPhysX2.Init()  More tetraverts than # of verts in Unity2Blender mesh!");					
