...
public class CBSoft : CBMesh, IObject {					//####DEV ####DESIGN: Based on CBMesh or CBSkin??
	...
	[HideInInspector]	public	string				_sNameDetachChunk;				// The name of our 'detached chunk' in Blender.  ('Breasts', 'Penis', 'Vagina') from a substring of our class name.  Must match Blender!!
	...
	public static CBSoft Create(CBody oBody, Type oTypeBMesh) { 
		string sNameDetachChunk = oTypeBMesh.Name.Substring(1);                            // Obtain the name of our detached body part ('Breasts', 'Penis', 'Vagina') from a substring of our class name.  Must match Blender!!  ###WEAK?
		CGame.gBL_SendCmd("CBody", "CBody_GetBody(" + oBody._nBodyID.ToString() + ").SoftBody_SeparateFromBody('" + sNameDetachChunk + "')");		// Separate the softbody from the source body.
		CBSoft oBSoft = (CBSoft)CBMesh.Create(null, oBody, "aMeshSoftBodies['" + sNameDetachChunk + "']", oTypeBMesh);								// Create the softbody mesh from the just-created Blender mesh.
		return oBSoft;
	}
	...
	public virtual void OnChangeGameMode(EGameModes eGameMode) {
		...
		switch (eGameMode) { 
			case EGameModes.Play:
				...
				CGame.gBL_SendCmd("CBody", "CBody_GetBody(" + _oBody._nBodyID.ToString() + ").SoftBody_CreateTempMesh(" + nVertTetras.ToString() + ")");     // Ask Blender to create a temporary mesh with just the number of verts we need.
				...
				CGame.gBL_SendCmd("CBody", "CBody_GetBody(" + _oBody._nBodyID.ToString() + ").SoftBody_ProcessTetraVerts('" + _sNameDetachChunk + "'," + _nRangeTetraPinHunt.ToString() + ")");		// Ask Blender select the tetraverts near the rim and skin them
