...
public class CActorChest : CActor {
	...
	public override void OnStart_DefineLimb() {
		...
		_aBones.Add(_oBoneAbdomenLower  = CBone.Connect(this, _oBodyBase._oActor_Pelvis._oBoneHip,	"abdomenLower",	    6));
		_aBones.Add(_oBoneAbdomenUpper	= CBone.Connect(this, _oBoneAbdomenLower,				    "abdomenUpper",	    6));
		_aBones.Add(_oBoneChestLower    = CBone.Connect(this, _oBoneAbdomenUpper,				    "chestLower",		8));
		_aBones.Add(_oBoneExtremity		= CBone.Connect(this, _oBoneChestLower,					    "chestUpper",	    6));		//###NOTE: While chestUpper acts as the real root bone it still has a non-kinematic rigid body and D6 joints going into and out of it.  However it has a D6 joint with a powerful position and rotational spring that makes it snap close to the position the player wants it at

		//###CHECK: Keep? _oBoneExtremity._oRigidBody.isKinematic = (CGame._GameMode == EGameModes.Configure);		//###HACK ###TEMP ####REVA If play no anim just set everything to kinematic... for temp cloth exploration
		//###OBS? _oBoneExtremity._oRigidBody.isKinematic = false;				// We need to disable kinematic on bone as we're driving it with our pin!

		//=== Init Hotspot ===
		//_oHotSpot = CHotSpot.CreateHotspot(this, transform, "Chest", true, G.C_Layer_HotSpot, C_SizeHotSpot_BodyNodes);
        //_oHotSpot = CHotSpot.CreateHotspot(this, _oBody.FindBone("chestUpper"), "Chest", true, new Vector3(0, 0, 0), C_SizeHotSpot_TorsoNodes);

        //=== Init CObj ===
        _oObj = new CObj("Chest", this);
        AddBaseActorProperties();						// The first properties of every CActor subclass are Pinned, pos & rot

        //_oObj.Add(EActorChest.Torso_LeftRight,	"Torso_LeftRight",	0,	-100,	100,	"");		//###BUG: Have to recalibrate to make new abdoment-centric bones look good.
        //_oObj.Add(EActorChest.Torso_UpDown,		"Torso_UpDown",		0,	-100,	100,	"");		//NOTE: Forced to enter crappy name because of new CObj restrictions with naming!
        //_oObj.Add(EActorChest.Torso_Twist,		"Torso_Twist",		0,	-100,	100,	"");		//###OBS
        //###PROBLEM: Base actor adds 'pinned' but chest is always pinned?

        //_oObj.Set(0, EActorChest.Pinned, 1);			// Manually set pinned to 1 on chest so body doesn't go floating into space.  This pin is the most powerful one because it has a strong positional AND rotational spring

        //if (CGame.EnableIdlePoseMovement)
    ...
    //###BROKEN
    //for (; ; ) {
    //	_oObj.Find(0, EActorChest.Torso_LeftRight)	.Randomize_SetNewRandomTarget(-C_RndPose_ValShift/2, C_RndPose_ValShift/2);		//###TUNE!!
    //	_oObj.Find(0, EActorChest.Torso_UpDown)		.Randomize_SetNewRandomTarget(-C_RndPose_ValShift, C_RndPose_ValShift);
    //	_oObj.Find(0, EActorChest.Torso_Twist)		.Randomize_SetNewRandomTarget(-C_RndPose_ValShift, C_RndPose_ValShift);
    ...
    //		_oObj.Find(0, EActorChest.Torso_LeftRight)	.Randomize_SmoothToTarget(nPlaceInCycle);
    //		_oObj.Find(0, EActorChest.Torso_UpDown)		.Randomize_SmoothToTarget(nPlaceInCycle);
    //		_oObj.Find(0, EActorChest.Torso_Twist)		.Randomize_SmoothToTarget(nPlaceInCycle);
    ...
    //---------------------------------------------------------------------------	COBJECT EVENTS
    //###OBS?
    //public void OnSet_Torso_LeftRight(float nValueOld, float nValueNew) {
    ...
    //   }
    //   public void OnSet_Torso_UpDown   (float nValueOld, float nValueNew) {
    ...
    //}
    //   public void OnSet_Torso_Twist    (float nValueOld, float nValueNew) {
