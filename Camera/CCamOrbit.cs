...
public class CCamOrbit : MonoBehaviour				//###DESIGN: Adopt a more capable 'super camera' package in Unity Store more capable than this one??
{													//###IMPROVE: Code always acts upon middle mouse button up/down even when focus is on GUI scrollbars... connect with GUI code to prevent this.
	...
	void LateUpdate()
	{
		if ( target )
		{
			...
			if ( distance < 0.1f )		//###MOD: So we can get closer!!
			{
				distance = 0.1f;
				nz = distance;
			}
